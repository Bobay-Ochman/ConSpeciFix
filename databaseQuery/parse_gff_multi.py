...
def processSpec(args):
	...
	f = None
	try:
		f=open(PATH_TO_QUERY_SPEC + sp + "/genomes/" + file +'.gff',"r")
	except:
		print PATH_TO_QUERY_SPEC + sp + "/genomes/" + file +'.gff'
		print 'file error'
		return
	...
	try:		
		...
		#print spec
		tmp[spec] = {}
		f=open(PATH_TO_QUERY_SPEC + sp + "/genomes/" + spec + ".fna","r")
		for l in f:
			if l[0] == ">":
				a=l.strip(">").strip("\n").split(" ")
				contig =  a[0]
	...
	for spec in dico:
		...
		if len(dico[spec]) > 4:
			try:
				h=open(PATH_TO_QUERY_SPEC + sp + "/genes/" + spec + ".fa" , "w")
